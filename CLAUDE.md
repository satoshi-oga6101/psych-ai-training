...
> 新カテゴリを足すと自動的にスキル軸ナビに現れる。安易に増やさず、既存に当てはまるか先に検討する。「統合トレーニング」「典型的な失敗パターン」は skillCategory に含めた（暫定）。厳密には演習タイプ寄りだが、現状はカテゴリとして運用中。
...
- **ブラウザストレージ（localStorage 等）は使わない。** 状態はメモリ上のみ。
...
- トップに簡単な「使い方」ガイド（初回訪問者向け）。
- 演習タイプ（統合トレーニング／典型的な失敗パターン 等）の表示・整理。
- URL で特定の演習を直接共有できるディープリンク（ハッシュ等。ブラウザストレージは使わない方針は維持）。
